...
Public Class ClsCtlKain : Implements InfProses
    Public Function cariData(kunci As String) As DataView Implements InfProses.cariData
        Try
            DTA = New OleDbDataAdapter("Select * from Kain where nama_kain " & "like '%" & kunci & "%'", BUKAKONEKSI)
            DTS = New DataSet()
    ...
    Public Function InsertData(Ob As Object) As OleDbCommand Implements InfProses.InsertData
        ...
        CMD = New OleDbCommand("insert into kain values('" & data.KodeKain & "','" & data.IdDetail & "','" _
                               & data.NamaKain & "','" & data.HargaKain & "','" & data.StokKain & "','" & data.Warnakain1 & "','" _
                               & data.Hargajual & "')", BUKAKONEKSI)
    ...
    Public Function deleteData(kunci As String) As OleDbCommand Implements InfProses.deleteData
        CMD = New OleDbCommand("delete from kain " _
                               & "where kode_kain ='" & kunci & "'", BUKAKONEKSI)
        CMD.CommandType = CommandType.Text
        CMD.ExecuteNonQuery()
        CMD = New OleDbCommand("", TUTUPKONEKSI)
        Return CMD
